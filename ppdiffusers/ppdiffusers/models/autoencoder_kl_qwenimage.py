...
class QwenImageCausalConv3d(nn.Conv3d):
    ...
    def __init__(
        ...
        self.padding = (0, 0, 0)
        self._updated_padding = (0, 0, 0)

    def forward(self, x, cache_x=None):
        padding = list(self._padding)
        if cache_x is not None and self._padding[4] > 0:
            cache_x = cache_x
            x = paddle.cat([cache_x, x], dim=2)
            padding[4] -= cache_x.shape[2]
        x = F.pad(x, padding)
        return super().forward(x)


class QwenImageRMS_norm(nn.Layer):
    r"""
    ...
    def __init__(self, dim: int, channel_first: bool = True, images: bool = True, bias: bool = False) -> None:
        ...
        self.scale = dim**0.5
        self.gamma = nn.Parameter(paddle.ones(shape))
        self.bias = nn.Parameter(paddle.zeros(shape)) if bias else 0.0
...
class QwenImageUpsample(nn.Upsample):
    ...
    Args:
        x (paddle.Tensor): Input tensor to be upsampled.

    Returns:
        paddle.Tensor: Upsampled tensor with the same data type as the input.
    """
...
class QwenImageResample(nn.Layer):
    r"""
    ...
    def __init__(self, dim: int, mode: str) -> None:
        ...
        if mode == "upsample2d":
            self.resample = nn.Sequential(
                QwenImageUpsample(scale_factor=(2.0, 2.0), mode="nearest"),
        ...
        if self.mode == "upsample3d":
            if feat_cache is not None:
                ...
                if feat_cache[idx] is None:
                    ...
                    if cache_x.shape[2] < 2 and feat_cache[idx] is not None and feat_cache[idx] == "Rep":
                        cache_x = paddle.cat([paddle.zeros_like(cache_x), cache_x], dim=2)
        ...
        if self.mode == "downsample3d":
            if feat_cache is not None:
                ...
                if feat_cache[idx] is None:
                    ...
                    cache_x = x[:, :, -1:, :, :].clone()
                    x = self.time_conv(paddle.cat([feat_cache[idx][:, :, -1:, :, :], x], 2))
                    feat_cache[idx] = cache_x
                    feat_idx[0] += 1
        return x


class QwenImageResidualBlock(nn.Layer):
    r"""
    ...
    def forward(self, x, feat_cache=None, feat_idx=[0]):
        ...
        if feat_cache is not None:
            ...
            if cache_x.shape[2] < 2 and feat_cache[idx] is not None:
                cache_x = paddle.cat([feat_cache[idx][:, :, -1, :, :].unsqueeze(2), cache_x], dim=2)
...
class QwenImageAttentionBlock(nn.Layer):
    r"""
...
class QwenImageMidBlock(nn.Layer):
    """
...
class QwenImageEncoder3d(nn.Layer):
    r"""
    A 3D encoder module.
...
class QwenImageUpBlock(nn.Layer):
    """
    ...
    def __init__(
        ...
        if upsample_mode is not None:
            self.upsamplers = nn.LayerList([QwenImageResample(out_dim, mode=upsample_mode)])
...
class AutoencoderKLQwenImage(ModelMixin, AutoencoderMixin, ConfigMixin):
    r"""
    ...
    def _encode(self, x: paddle.Tensor):
    ...
    def _decode(self, z: paddle.Tensor, return_dict: bool = True):
        ...
        out = paddle.clamp(out, min=-1.0, max=1.0)
        self.clear_cache()
        if not return_dict:
            return (out,)
    ...
    @apply_forward_hook
    def decode(self, z: paddle.Tensor, return_dict: bool = True) -> DecoderOutput | paddle.Tensor:
        r"""
        ...
        if self.use_slicing and z.shape[0] > 1:
            ...
            decoded = paddle.cat(decoded_slices)
        else:
    ...
    def tiled_encode(self, x: paddle.Tensor) -> AutoencoderKLOutput:
        ...
        Args:
            x (`paddle.Tensor`): Input batch of videos.
        ...
        for i in range(0, height, self.tile_sample_stride_height):
            ...
            for j in range(0, width, self.tile_sample_stride_width):
                ...
                for k in range(frame_range):
                    ...
                    time.append(tile)
                row.append(paddle.cat(time, dim=2))
            rows.append(row)
        self.clear_cache()
        ...
        for i, row in enumerate(rows):
            ...
            result_rows.append(paddle.cat(result_row, dim=-1))

        enc = paddle.cat(result_rows, dim=3)[:, :, :, :latent_height, :latent_width]
        return enc

    def tiled_decode(self, z: paddle.Tensor, return_dict: bool = True) -> DecoderOutput | paddle.Tensor:
        r"""
        ...
        dec = paddle.cat(result_rows, dim=3)[:, :, :, :sample_height, :sample_width]
