...
class FlowMatchEulerDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def time_shift(self, mu: float, sigma: float, t: paddle.Tensor) -> paddle.Tensor:
        """
    ...
    def set_timesteps(
        ...
        sigmas = paddle.to_tensor(sigmas).to(dtype=paddle.float32)
        ...
        if self.config.invert_sigmas:
            sigmas = 1.0 - sigmas
            timesteps = sigmas * self.config.num_train_timesteps
            sigmas = paddle.cat([sigmas, paddle.ones(1)])
        else:
            sigmas = paddle.cat([sigmas, paddle.zeros(1)])
    ...
    def index_for_timestep(
        self,
        timestep: Union[float, paddle.Tensor],
        schedule_timesteps: Optional[paddle.Tensor] = None,
    ) -> int:
        """
        ...
        Args:
            ...
            schedule_timesteps (`paddle.Tensor`, *optional*):
    ...
    def _init_step_index(self, timestep: Union[float, paddle.Tensor]) -> None:
        if self.begin_index is None:
    ...
    def step(
        ...
        per_token_timesteps: Optional[paddle.Tensor] = None,
        ...
        Args:
            ...
            per_token_timesteps (`paddle.Tensor`, *optional*):
    ...
    def _convert_to_karras(self, in_sigmas: paddle.Tensor, num_inference_steps: int) -> paddle.Tensor:
        """
        Construct the noise schedule as proposed in [Elucidating the Design Space of Diffusion-Based Generative
        Models](https://huggingface.co/papers/2206.00364).

        Args:
            in_sigmas (`paddle.Tensor`):
    ...
    def _time_shift_exponential(self, mu: float, sigma: float, t: paddle.Tensor) -> paddle.Tensor:
        return math.exp(mu) / (math.exp(mu) + (1 / t - 1) ** sigma)

    def _time_shift_linear(self, mu: float, sigma: float, t: paddle.Tensor) -> paddle.Tensor:
