...
from ppdiffusers.models.attention_processor import LoRAAttnAddedKVProcessor
...
def main():
    ...
    def collate_fn(examples):
        ...
        clip_pixel_values = paddle.stack([example["clip_pixel_values"] for example in examples]).cast("float32")
    ...
    for epoch in range(args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            timesteps = timesteps.astype("int64")
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                if num_processes > 1 and args.gradient_checkpointing:
                    fused_allreduce_gradients(unet_lora_layers.parameters(), None)
                optimizer.step()
                lr_scheduler.step()
