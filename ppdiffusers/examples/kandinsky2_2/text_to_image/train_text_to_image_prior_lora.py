...
def main():
    ...
    prior_lora_attn_procs = {}
    for name, attn_processor in prior.attn_processors.items():
        prior_lora_attn_procs[name] = LoRAAttnProcessor(
            hidden_size=2048,
            rank=args.lora_rank,
        )

    prior.set_attn_processor(prior_lora_attn_procs)
    prior_lora_layers = AttnProcsLayers(prior.attn_processors)
    ...
    optimizer = AdamW(
        ...
        parameters=prior_lora_layers.parameters(),
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                if num_processes > 1 and args.gradient_checkpointing:
                    fused_allreduce_gradients(prior_lora_layers.parameters(), None)
                optimizer.step()
                lr_scheduler.step()
                ...
                if is_main_process:
                    ...
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        LoraLoaderMixin.save_lora_weights(
                            ...
                            unet_lora_layers=prior_lora_layers,
                        )
                ...
                if global_step >= args.max_train_steps:
                    break
        if is_main_process:
            prior_scheduler = UnCLIPScheduler(beta_schedule="squaredcos_cap_v2", prediction_type="sample")
            if args.validation_prompt is not None and epoch % args.validation_epochs == 0 and epoch > 0:
                ...
                pipeline = AutoPipelineForText2Image.from_pretrained(
                    ...
                    prior_prior=unwrap_model(prior),
